...
export default function OwnerPosts() {
  ...
  const [pendingCoverBase64, setPendingCoverBase64] = useState('');
  ...
  const handleFileSelect = (e) => {
    ...
    if (!file.type.startsWith('image/')) {
      setNoticeModal({ open: true, title: 'Lỗi chọn file', message: 'Vui lòng chọn file định dạng hình ảnh (JPG, PNG, WebP,...).', type: 'error' });
      return;
    }
  ...
  const handleSavePost = async () => {
    ...
    try {
      ...
      // If user selected a new image from device, upload it to Media Library first
      if (pendingCoverBase64 && selectedVenueId) {
        try {
          const uploadCategory = contentType === 'PROMOTION' ? 'PROMOTION' : (contentType === 'EVENT' || contentType === 'TOURNAMENT' ? 'EVENT' : 'VENUE');
          const uploadRes = await uploadOwnerMedia(selectedVenueId, {
            base64_image: pendingCoverBase64,
            image_type: uploadCategory,
            title: `Ảnh bìa: ${title.trim()}`,
            status: 'PUBLISHED'
          });

          const createdImg = uploadRes?.data?.[0];
          if (createdImg) {
            finalCoverImageId = createdImg.image_id;
            finalCoverImageUrl = createdImg.medium_url || createdImg.large_url || createdImg.original_url || createdImg.image_url;
          }
        } catch (uploadErr) {
          console.error('Failed to upload image to venue media library:', uploadErr);
        }
      }

      // Filter out raw base64 Data URLs from post payload cover_image_url to avoid huge DB text if not needed
      const cleanCoverUrl = (finalCoverImageUrl && !finalCoverImageUrl.startsWith('data:'))
        ? finalCoverImageUrl
        : (coverImageUrl && !coverImageUrl.startsWith('data:') ? coverImageUrl : null);
      ...
      const payload = {
        ...
        cover_image_id: finalCoverImageId || null,
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {loadingPosts ? (
        ...
        <div className="space-y-4">
          {posts.map((post) => {
            const rawCoverUrl = post.cover_image_url || post.cover_image?.medium_url || post.cover_image?.large_url || post.cover_image?.image_url || post.cover_image?.original_url;
            const coverUrl = rawCoverUrl ? getImageUrl(rawCoverUrl, post.post_id) : null;
          ...
          <div className="bg-surface rounded-2xl max-w-3xl w-full p-6 space-y-4 shadow-2xl border border-border-subtle-medium max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {/* IMAGE PREVIEW BOX */}
                {coverImageUrl ? (
                  <div className="relative group rounded-2xl overflow-hidden border border-border-subtle-medium bg-gray-100 max-h-48 flex items-center justify-center">
                    <img src={getImageUrl(coverImageUrl, 'preview')} alt="Cover preview" className="w-full h-44 object-cover" />
                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                      ...
                      <button
                        ...
                        className="bg-rose-600 text-white px-3 py-1.5 rounded-xl text-xs font-bold hover:bg-rose-700 flex items-center gap-1.5 shadow-md"
                      >
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <label className="cursor-pointer inline-flex items-center gap-1.5 px-3.5 py-2 bg-brand-orange text-white text-xs font-bold rounded-xl hover:bg-brand-orange/90 transition-all shadow-xs">
                    <Upload size={15} />
                    Upload ảnh từ máy
                    <input type="file" accept="image/*" className="hidden" onChange={handleFileSelect} />
                  </label>
                ...
                {pendingCoverBase64 && (
                  <p className="text-[11px] text-emerald-600 font-semibold flex items-center gap-1">
                    ✓ Ảnh vừa chọn từ máy sẽ tự động được tải lên và lưu vào <span className="underline font-bold">Thư viện hình ảnh</span> khi bạn bấm "{editingPostId ? 'Lưu cập nhật' : 'Tạo mới'}".
                  </p>
                )}
              </div>
          ...
          <div className="bg-surface rounded-2xl max-w-lg w-full p-5 space-y-4 shadow-2xl border border-border-subtle-medium max-h-[80vh] overflow-y-auto">
            ...
            {loadingImages ? (
              ...
              <div className="grid grid-cols-3 gap-3">
                {venueImages.map((img) => {
                  const rawImgUrl = img.thumbnail_url || img.medium_url || img.large_url || img.original_url || img.image_url || img.cover || img.avatar;
                  const formattedUrl = getImageUrl(rawImgUrl, img.image_id);
                  return (
                    <div
                      key={img.image_id}
                      onClick={() => {
                        setCoverImageId(img.image_id);
                        setCoverImageUrl(rawImgUrl);
                        setIsMediaSelectorOpen(false);
                      }}
                      className="cursor-pointer border-2 rounded-xl overflow-hidden aspect-video hover:border-brand-orange transition-all bg-gray-100"
                    >
                      <img src={formattedUrl} alt={img.title || 'Venue media'} className="w-full h-full object-cover" />
                    </div>
                  );
                })}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-dark/70 backdrop-blur-xs">
          <div className="bg-surface rounded-2xl max-w-2xl w-full p-6 space-y-4 shadow-2xl border border-border-subtle-medium max-h-[85vh] overflow-y-auto">
            ...
            {viewingPost.cover_image_url && (
              <img src={getImageUrl(viewingPost.cover_image_url, viewingPost.post_id)} alt={viewingPost.title} className="w-full h-56 object-cover rounded-xl" />
            )}
